<footer class="site-footer">
            <div class="footer-profile-area">
              <div class="footer-profile-table">
                <div class="footer-profile-side">
                  <div class="profile-content">
                    <div class="selfintro">
                      <div class="profimg">
                        <img src="{{ $glasses }}" alt="" class="profpic">
                      </div>
                      <div class="prof-meta">
                        <div class="profname">
                          <strong>Shota Shimizu</strong>
                        </div>
                        <div class="profsns">
                          <a href="https://twitter.com/shota_s0608" class="proficon"><i class="fab fa-twitter"></i></a>
                          <a href="https://www.instagram.com/iec_shota000" class="proficon"><i class="fab fa-instagram"></i></a>
                          <a href="https://github.com/shimish000" class="proficon"><i class="fab fa-github"></i></a>
                        </div>
                      </div>
                    </div>
                    <div class="proftext">
                      <b>Birth:</b>  1995.06.08 <br>
                      <b>Birthplace:</b>  Osaka <br>
                      <b>Specialty:</b>  Ball game(sports) <br>
                      <b>Development record:</b>  Haml・SCSS・Ruby・Ruby on Rails・JavaScript・jQuery・GitHub・AWS

                    </div>
                  </div>
                </div>
                <div class="footer-profile-main">
                  <div class="pf profile-1"></div>
                  <div class="pf profile-2"></div>
                  <div class="pf profile-3"></div>
                </div>
              </div>
            </div>
            <div class="site-bottom">
              <div class="site-bottom__table">
                <nav class="footer-social-link">
                  <ul class="social-icon">
                    <a href="https://twitter.com/shota_s0608"><li class="fabli"><i class="fab fa-twitter"></i></li></a>
                    <a href="https://www.instagram.com/iec_shota000"><li class="fabli"><i class="fab fa-instagram"></i></li></a>
                    <a href="https://github.com/shimish000"><li class="fabli"><i class="fab fa-github"></i></li></a>
                  </ul>
                </nav>
                <div class="site-info">
                  <div class="site-credit">
                      Created by Shota Shimizu | Html/Css Cording with PHP/Laravel
                      
                  </div>
                </div>
              </div>
            </div>
          </footer>
        </div>
      </div>